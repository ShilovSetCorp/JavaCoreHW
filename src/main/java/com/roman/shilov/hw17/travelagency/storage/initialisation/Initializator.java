package com.roman.shilov.hw17.travelagency.storage.initialisation;
...
import com.roman.shilov.hw17.travelagency.storage.initialisation.dom.XmlReader;
import com.roman.shilov.hw17.travelagency.storage.initialisation.jaxb.JaxbReader;
import com.roman.shilov.hw17.travelagency.storage.initialisation.sax.XmlSaxReader;
import com.roman.shilov.hw17.travelagency.storage.initialisation.stax.XmlStaxReader;
    ...
    public static List<BaseCountry> initCountriesAndCities(String filePath, DataSourceType sourceType) throws Exception {
        ...
        switch (sourceType) {
            ...
            case XML_FILE: {
                switch (parserType){
                    case DOM: {
                        sourceReader = (filePath1) -> XmlReader.readDataFromFileViaLambda(filePath);
                        break;
                    }
                    case SAX: {
                        sourceReader = (filePath1) -> XmlSaxReader.readDataFromFileViaLambda(filePath);
                        break;
                    }
                    case STAX:{
                        sourceReader = (filePath1) -> XmlStaxReader.readDataFromFileViaLambda(filePath);
                        break;
                    }
                    case JAXB: {
                        sourceReader = (filePath1) -> JaxbReader.readDataFromFileViaLambda(filePath);
                        break;
                    }
                }
                break;
